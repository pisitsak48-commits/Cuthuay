            ...
            <Card>
              ...
              <div className="px-5 pb-5 space-y-4 text-sm text-theme-text-secondary">
                <p className="text-xs text-theme-text-muted leading-relaxed">
                  เพิ่มบอทในกลุ่มไลน์ แล้วตั้ง Webhook URL เป็น <code className="text-accent/90  tracking-tight bg-surface-200 px-1 rounded">HTTPS://โดเมนของคุณ/api/line/webhook</code>
                  {' '}และใส่ Channel secret ในตัวแปรสภาพแวดล้อม <code className=" tracking-tight bg-surface-200 px-1 rounded">LINE_CHANNEL_SECRET</code> ของเซิร์ฟเวอร์ API
                </p>
                  ...
                  <>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full h-10 rounded-lg bg-surface-200 border border-border px-3 text-sm  tracking-tight"
                        />
                      </div>
                      <div>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full rounded-lg bg-surface-200 border border-border px-3 py-2 text-xs  tracking-tight text-theme-text-primary focus:outline-none focus:ring-2 focus:ring-[var(--color-accent-ring)]"
                      />
                    </div>
                            ...
                            {lineLogs.length === 0 ? (
                              ...
                              lineLogs.map((row) => (
                                <tr key={row.id} className="border-t border-border/80">
                                  <td className="py-1.5 px-2 whitespace-nowrap text-theme-text-muted ">
                                    {new Date(row.received_at).toLocaleString('th-TH', { hour: '2-digit', minute: '2-digit', day: 'numeric', month: 'short' })}
                                  </td>
                                  <td className="py-1.5 px-2  tracking-tight">{row.status}</td>
                                  <td className="py-1.5 px-2 text-right ">{row.inserted_count}</td>
