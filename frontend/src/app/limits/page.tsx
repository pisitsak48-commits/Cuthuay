...
function LimitTable({ limits, customers, dealers, dealerLimits, selectedId, tab, onSelect, onDelete }: LimitTableProps) {
 ...
 l.id === selectedId ? 'bg-[var(--color-nav-active-bg)]' : 'hover:bg-[var(--bg-hover)]'
            ...
            >
              ...
              <td className="px-3 py-2 text-center text-theme-text-secondary ">
              ...
              <td className="px-3 py-2 text-right">
                <button
                  ...
                  className="text-risk-high hover:text-[var(--color-semantic-danger-muted)] px-1"
                  title="ลบ"
                >✕</button>
              </td>
            </tr>
...
function RightForm({ tab, customers, dealers, dealerLimits, roundId, selectedLimitId, onSave }: RightFormProps) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {msg && (
        <p className={`text-sm text-center mt-1 ${msg.includes('ผิดพลาด') ? 'text-loss' : 'text-[var(--color-semantic-success-muted)]'}`}>
          {msg}
        </p>
      )}
    </div>
