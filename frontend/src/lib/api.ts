...
export const roundsApi = {
  ...
  importRound:  (data: Record<string, unknown>) =>
    api.post('/rounds/import', data, { timeout: 300_000 }),
  /** ตรวจสอบก่อนนำเข้า — ซ้ำ id / วันออก */
  importPreview: (data: Record<string, unknown>) =>
    api.post<ImportPreviewResponse>('/rounds/import-preview', data, { timeout: 300_000 }),
};

// ─── Bets ─────────────────────────────────────────────────────────────────────
export type RoundBetsSummary = {
  by_type: Partial<Record<string, number>>;
  total: number;
};

export type BetsListResponse = { bets: Bet[]; total?: number };

export const betsApi = {
  list:   (roundId: string, customerId?: string) =>
    api.get<BetsListResponse>('/bets', {
      params: { round_id: roundId, ...(customerId ? { customer_id: customerId } : {}) },
    }),
  roundSummary: (roundId: string) =>
    api.get<RoundBetsSummary>('/bets/summary', { params: { round_id: roundId } }),
