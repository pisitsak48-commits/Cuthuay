...
function parseToken(token: string): { number: string; amountStr: string } | null {
  ...
  // LINE shorthand for 3 digits:
  // 123-100-120-130  → 123=100*120*130
  // 123+100-520=20   → 123=100*520*20
  m = s.match(/^(\d{3})\s*-\s*(\d+)\s*-\s*(\d+)\s*-\s*(\d+)$/);
  if (m) return { number: m[1], amountStr: `${m[2]}+${m[3]}+${m[4]}` };
  m = s.match(/^(\d{3})\s*\+\s*(\d+)\s*-\s*(\d+)\s*=\s*(\d+)$/);
  if (m) return { number: m[1], amountStr: `${m[2]}+${m[3]}+${m[4]}` };
...
export function parseLineBetsText(text: string): LineImportResult {
  ...
  for (let lineIdx = 0; lineIdx < rawLines.length; lineIdx++) {
    ...
    // ── 3 ตัว shorthand จากไลน์: 123-100-120-130 / 123+100-520=20 ──
    // ต้องมาก่อนบล็อก "เลขคู่สองตัวราคาเดียวกัน" ไม่งั้น 123-100-120-130 จะถูกอ่านเป็นเลข 123 และ 100
    if (/^\d{3}\s*-\s*\d+\s*-\s*\d+\s*-\s*\d+$/.test(line) || /^\d{3}\s*\+\s*\d+\s*-\s*\d+\s*=\s*\d+$/.test(line)) {
      const p = parseToken(line);
      if (p) {
