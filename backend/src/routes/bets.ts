...
// ─── GET /api/bets?round_id=xxx&customer_id=yyy (optional) ─────────────────────
...
/** สรุปยอดรวมทั้งงวด (ไม่โหลดแถวโพย) — ใช้แผงสรุปหน้าคีย์ */
router.get('/summary', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const roundId = z.string().uuid().parse(req.query.round_id);
    const result = await query<{ bet_type: BetType; total: string }>(
      `SELECT bet_type, COALESCE(SUM(amount), 0)::text AS total
       FROM bets WHERE round_id = $1
       GROUP BY bet_type`,
      [roundId],
    );
    const by_type: Partial<Record<BetType, number>> = {};
    let total = 0;
    for (const row of result.rows) {
      const amt = Number(row.total);
      by_type[row.bet_type] = amt;
      total += amt;
    }
    res.json({ by_type, total });
  } catch (err) {
    next(err);
  }
});
        ...
        for (const b of bets) {
          ...
          const r = await client.query<BetRow>(
            `INSERT INTO bets (round_id, number, bet_type, amount, payout_rate, customer_id, customer_ref, sheet_no, created_by, sort_order, import_batch_id, segment_index, created_at)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, COALESCE($10, EXTRACT(EPOCH FROM clock_timestamp()) * 1000), $11, COALESCE($12, 0), COALESCE($13::timestamptz, NOW())) RETURNING *`,
            [
              round_id,
              b.number,
              b.bet_type,
