...
async function buildRoundExportBundle(roundId: string) {
  ...
  const betsRes = await query(
    `SELECT id, round_id, number, bet_type, amount, payout_rate, customer_id, customer_ref, sheet_no,
            sort_order, import_batch_id, segment_index, created_at, updated_at
     FROM bets WHERE round_id = $1
     ORDER BY COALESCE(sort_order, EXTRACT(EPOCH FROM created_at) * 1000) ASC, id ASC`,
    [roundId],
  );
...
async function importOneRoundPack(
  ...
  return withTransaction(async (client) => {
    ...
    for (let i = 0; i < body.bets.length; i++) {
      ...
      const updatedAt =
        bet.updated_at != null && String(bet.updated_at) !== ''
          ? String(bet.updated_at)
          : createdAt;
      const rawSort = bet.sort_order;
      const hasExplicitSort =
        rawSort != null && rawSort !== '' && !Number.isNaN(Number(rawSort));
      const sortOrder = hasExplicitSort
        ? Number(rawSort)
        : new Date(createdAt).getTime() + i * 0.001;
      const importBid =
        bet.import_batch_id != null &&
        String(bet.import_batch_id) !== '' &&
        z.string().uuid().safeParse(String(bet.import_batch_id)).success
          ? String(bet.import_batch_id)
          : null;
      const seg =
        bet.segment_index != null && String(bet.segment_index) !== ''
          ? Math.max(0, parseInt(String(bet.segment_index), 10) || 0)
          : 0;

      await client.query(
        `INSERT INTO bets (
           id, round_id, number, bet_type, amount, payout_rate, customer_id, customer_ref, sheet_no,
           created_by, sort_order, import_batch_id, segment_index, created_at, updated_at
         )
        ...
        [
          ...
          userId, sortOrder, importBid, seg, createdAt, updatedAt,
        ],
      );
      betCount++;
    }
